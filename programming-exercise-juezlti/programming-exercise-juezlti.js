...
/****************************************************************/
var JWT_TOKEN;
const BASE_URL = "http://fgpe.dcc.fc.up.pt/api";
/****************************************************************/
/** @function do_auth
 *  
 * Internal function to does the authentication
 *  @param {string} ID  The exercise ID   */
async function do_auth(EMAIL, PASWORD, callback) {
...
class ProgrammingExercise {
    ...
    getSolutions() {
         ...
         * @param {string} EMAIL  AuthorKit's EMAIL
         * @param {string} PASS  AuthorKit's password  */
    static loadRemoteExerciseAuthorkit(ID, EMAIL, PASS) {
    ...
    static async loadRemoteExercise(URI) {
        try {
            const tempDir = os.tmpdir();
            await pipeline(
                got.stream(URI, {
